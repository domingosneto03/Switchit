...
import '../screens/home/search/view_model/user_data_model.dart';
...
class NetworkFirestoreController {
  ...
  Future<bool> addUserToCloud(String name, String surname, String email) async {
    ...
    await users.add({
      ...
      debugPrint(
          "FirebaseFirestore (addUserToCloud): User Added with id: ${value
              .id}");
  ...
  Future<bool> addItemCurrentUserToCloud(String name, String description,
      String location, String imageUrl) async {
    CollectionReference users =
    FirebaseFirestore.instance.collection(TableCloudUser.name);
  ...
  Future<List<UserDataModel>> getAllUsersFromCloud(var userDocId) async {
    CollectionReference users =
    FirebaseFirestore.instance.collection(TableCloudUser.name);

    var userDocId = await DatabaseRealm().getUserDocId();

    List<UserDataModel> users_list = [];
    ...
    for (var doc in data.docs) {
      String id = doc.id;
      String name = doc.get(TableCloudItem.fieldItemName);
      String surname = doc.get(TableCloudItem.fieldItemDescription);
      String email = doc.get(TableCloudItem.fieldItemLocation);
      Future<List<ItemDataModel>> items = getItemsUserCloud(doc.id);

      debugPrint(
          "FirebaseFirestore (getAllUsersFromCloud): UserDataModel-> name: $name, surname: $surname, email:$email, items:$items");

      users_list.add(
          UserDataModel(id, name, surname, email, items as List<ItemDataModel>));
    }

    debugPrint("FirebaseFirestore (getAllUsersFromCloud): Success");

    return users_list;
  }

  Future<List<ItemDataModel>> getItemsUserCloud(var userDocId) async {
    CollectionReference users =
    FirebaseFirestore.instance.collection(TableCloudUser.name);
